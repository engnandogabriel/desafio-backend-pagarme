export default class ValidateDate {
    ...
    private setValidateDate(date: string) {
        ...
        if (!dateRegex.test(date)) throw new Error('Date is invalid');
        const [month, year] = date.split('/').map(Number);
        const currentMonth = new Date().getMonth() + 1;
        const currentYear = new Date().getFullYear() % 100;

        if (year < currentYear || (year === currentYear && month < currentMonth) || month < 1 || month > 12) throw new Error('Date is invalid');
